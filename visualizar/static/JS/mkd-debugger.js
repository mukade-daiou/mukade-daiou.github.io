...
function codeRun(tab) {
    if (nowIndex == maxIndex) {
        clearInterval(timer);
        if (nowIndex)
            document.getElementById("index_" + (nowIndex - 1)).style.color = "black";
        return;
    }
    players[tab - 1].update(
        document.getElementById("index_" + nowIndex).textContent
    );
    if (nowIndex)
        document.getElementById("index_" + (nowIndex - 1)).style.color = "black";
...
class Player {
    ...
    checkPos(x, y) {
        ...
        return (
            x + 1 && x < map.length && y + 1 && y < map[0].length && map[x][y] != "x"
        );
    }
    reset() {
    ...
    rotate() {
        ...
        let centerX = this.x * 30 + 15,
            centerY = this.y * 30 + 15;
        ctx.save();
...
let players = [];
$(document).ready(() => {
    for (let i = 1; i <= 3; ++i) createMap(i);
});

function bytesUpdate() {
    let val = document.getElementById("code").value;
    document.getElementById("bytes").textContent = val.length;
}